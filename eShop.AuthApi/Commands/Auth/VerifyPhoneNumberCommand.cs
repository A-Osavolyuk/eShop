...
internal sealed record VerifyPhoneNumberCommand(VerifyPhoneNumberRequest Request) : IRequest<Result<VerifyPhoneNumberResponse>>;
    ...
    ISmsService smsService) : IRequestHandler<VerifyPhoneNumberCommand, Result<VerifyPhoneNumberResponse>>
{
    ...
    public async Task<Result<VerifyPhoneNumberResponse>> Handle(VerifyPhoneNumberCommand request, CancellationToken cancellationToken)
    {
        ...
        if (user is null)
        {
            return new Result<VerifyPhoneNumberResponse>(new NotFoundException($"Cannot find user with phone number ${request.Request.PhoneNumber}"));
        }
