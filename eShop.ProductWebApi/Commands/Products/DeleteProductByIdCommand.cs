...
namespace eShop.ProductWebApi.Commands.Products
{
    ...
    public class DeleteProductByIdCommandHandler(
        ...
        IRequestClient<DeleteCommentsRequest> requestClient)
        : IRequestHandler<DeleteProductByIdCommand, Result<DeleteProductResponse>>
    {
        private readonly ILogger<DeleteProductByIdCommandHandler> logger = logger;
        private readonly ProductDbContext context = context;
        private readonly IRequestClient<DeleteCommentsRequest> requestClient = requestClient;

        public async Task<Result<DeleteProductResponse>> Handle(DeleteProductByIdCommand request,
            CancellationToken cancellationToken)
        {
            var actionMessage = new ActionMessage("delete product with ID {0}", request.Request.Id);
            try
            {
                logger.LogInformation("Attempting to delete product with ID {id}. Request ID {requestID}",
                ...
                var response = await requestClient.GetResponse<DeleteCommentResponse>(
                    new DeleteCommentsRequest()
                    {
                        ProductId = request.Request.Id
                    }, cancellationToken);

                if (!response.Message.IsSucceeded)
                {
                    return logger.LogErrorWithException<DeleteProductResponse>(
                        new FailedRpcException(response.Message.Message), actionMessage, request.Request.RequestId);
                }

                context.Products.Remove(product);
                await context.SaveChangesAsync(cancellationToken);

                logger.LogInformation("Product with ID {id} was successfully deleted. Request ID {requestId}",
                    request.Request.Id, request.Request.RequestId);
            ...
            {
                return logger.LogErrorWithException<DeleteProductResponse>(dbUpdateException, actionMessage,
                    request.Request.RequestId);
            }
            catch (Exception ex)
            {
                return logger.LogErrorWithException<DeleteProductResponse>(ex, actionMessage,
                    request.Request.RequestId);
            }
        }
    }
