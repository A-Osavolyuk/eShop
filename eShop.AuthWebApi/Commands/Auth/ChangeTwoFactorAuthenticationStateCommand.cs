...
{
    public record ChangeTwoFactorAuthenticationStateCommand(ChangeTwoFactorAuthenticationRequest Request) : IRequest<Result<ChangeTwoFactorAuthenticationResponse>>;

    public class ChangeTwoFactorAuthenticationStateCommandHandler(
        AppManager appManager,
        ILogger<ChangeTwoFactorAuthenticationStateCommandHandler> logger) : IRequestHandler<ChangeTwoFactorAuthenticationStateCommand, Result<ChangeTwoFactorAuthenticationResponse>>
    {
        ...
        public async Task<Result<ChangeTwoFactorAuthenticationResponse>> Handle(ChangeTwoFactorAuthenticationStateCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                return logger.LogErrorWithException<ChangeTwoFactorAuthenticationResponse>(ex, actionMessage, request.Request.RequestId);
            }
        }
    }
}
