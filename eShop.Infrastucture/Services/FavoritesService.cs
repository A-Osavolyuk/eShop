    ...
    IConfiguration configuration) : IFavoritesService
{
    ...
    public async ValueTask<ResponseDTO> GetFavoritesAsync(GetFavoritesRequest request) => await clientService.SendAsync(
        new RequestDto(Url: $"{configuration["Services:Gateway"]}/api/v1/Favorites/get-favorites", Data:request, Method: HttpMethods.GET));
